...
export function GroupEditor({
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Kiểm tra lại thông tin nhóm nhé.');
    }
  }
  return (
    <form className={styles.editor} onSubmit={submit} aria-label="Thông tin nhóm chơi">
          ...
          <div className={styles.player} key={player.id}>
            ...
            <div>
              ...
              <input
                ...
                maxLength={30}
                placeholder={
                  index === 0 ? 'Ví dụ: Minh' : index === 1 ? 'Ví dụ: Linh' : 'Tên hoặc biệt danh'
                }
                autoComplete="off"
              />
            </div>
      ...
      >
        <Plus size={17} /> {players.length >= 8 ? 'Đã đủ 8 thành viên' : 'Thêm thành viên'}
      </button>
      {error && (
