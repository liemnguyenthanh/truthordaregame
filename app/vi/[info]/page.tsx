  ...
  'chinh-sach-thanh-toan': {
    ...
    sections: [
      ...
      },
      {
        ...
        text: 'Các bộ hiện tại có ít câu minh họa để thử trải nghiệm. Thanh toán chỉ hoạt động sau khi chủ ứng dụng cấu hình ngân hàng, SePay, dữ liệu sản phẩm và kênh hỗ trợ. Không phát sinh thanh toán khi hệ thống chưa sẵn sàng.',
      },
    ],
  },
  'quyen-rieng-tu': {
...
export async function generateMetadata({ params }: { params: Promise<{ info: string }> }) {
  const info = (await params).info;
  const p = pages[info];
  return p ? pageMetadata(p.title, p.sections[0].text, `/vi/${info}`) : {};
}
