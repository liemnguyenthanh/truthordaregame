...
function completed(input: GenerationInput): GeneratedPack {
  return {
    ...
    pack: {
      id: `ai-${id}`,
      slug: `ai-${id}`,
      title: 'Hội cuối tuần · Vui vẻ',
      description: 'Bộ thử nghiệm API',
      tier: 'free',
      questionFile: '',
      contentVersion: '1',
      questionCount: 4,
      ...
      questions: [
        {
          id: 't1',
          type: 'truth',
          text: 'An, bạn thích điều gì ở Bình?',
          playerId: input.group.players[0].id,
          partnerId: input.group.players[1].id,
        },
        {
          id: 't2',
          type: 'truth',
          text: 'Bình, kỷ niệm nào cùng An khiến bạn vui?',
          playerId: input.group.players[1].id,
          partnerId: input.group.players[0].id,
        },
        {
          id: 'd1',
          type: 'dare',
          text: 'An, nói một lời cảm ơn với Bình.',
          playerId: input.group.players[0].id,
          partnerId: input.group.players[1].id,
        },
        {
          id: 'd2',
          type: 'dare',
          text: 'Bình, diễn tả một con vật để An đoán.',
          playerId: input.group.players[1].id,
          partnerId: input.group.players[0].id,
        },
      ],
...
async function seedGroup(page: Page) {
  await page.addInitScript((value) => {
    localStorage.setItem('tod:group:v1', JSON.stringify(value));
  }, group);
}
test('MOCK AI: group and adult mood consent sent, generated actor questions persist on reload', async ({
  page,
}) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname === '/api/generations' && route.request().method() === 'POST') {
      ...
      expect(input.mood).toBe('flirty');
      expect(input.adultsConfirmed).toBe(true);
      expect(route.request().headers()['idempotency-key']).toBeTruthy();
      generation = completed(input);
  ...
  expect(
    await page.evaluate(
      (key) => JSON.parse(localStorage.getItem(`tod:generated:v1:${key}`) || 'null')?.status,
      id,
    ),
  ).toBe('complete');
});
test('MOCK AI: lost response retries with same idempotency key and no fake pack', async ({
  page,
}) => {
  await seedGroup(page);
  const keys: string[] = [];
  ...
  await page.route('**/api/generations/**', async (route) => {
    ...
    await route.fulfill({
      json: {
        generation:
          reads === 1
            ? { id, group, mood: 'friendly', status: 'pending', createdAt: full.createdAt }
            : full,
      },
    });
  });
    ...
    await route.fulfill({
      json: {
        generation: {
          ...
          status: 'failed',
          createdAt: '2026-09-17T12:00:00Z',
          error: 'AI không tạo đủ câu hợp lệ.',
        },
      },
    });
  });
