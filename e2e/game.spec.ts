...
async function readProgress(page: Page, packId: string) {
  return page.evaluate(id => JSON.parse(localStorage.getItem(`tod:game:v1:${id}`) || 'null'), packId);
}
...
test('free game draws, persists exactly across refresh, exhausts truth and continues dare', async ({ page }) => {
  ...
  await page.reload();
  await expect.poll(async () => (await readProgress(page, 'friends-free'))?.currentId).toBe(first.currentId);
...
test('premium fixed 4/4 trial leaves eighth card readable; next request reveals paywall', async ({ page }) => {
...
test('unconfigured checkout shows honest unavailable state and no paid success', async ({ page, request }) => {
  const product = await request.get('/api/products/friends-premium');
  test.skip(product.status() !== 503, 'This scenario is specific to a local server without payment configuration.');
  await page.goto('/vi/thanh-toan?pack=friends-premium');
  await expect(page.getByRole('main').getByRole('alert')).toContainText(/chưa được cấu hình|chưa sẵn sàng|gián đoạn/);
...
test('MOCK API: checkout QR to verified paid response, recovery code and continue', async ({ page }) => {
  // Browser-only contract simulation; this test does not exercise SePay or move money.
  let paid = false;
  const recoveryCode = 'TEST-RECOVERY-CODE';
  const order = { id: 'test-order', packId: 'friends-premium', status: 'pending', amountVnd: 30000, currency: 'VND', expiresAt: new Date(Date.now() + 900_000).toISOString(), paymentCode: 'TODTEST1234', qrUrl: '/icons/icon-192.png', bankName: 'Ngân hàng thử nghiệm', accountNumber: '123456789', accountName: 'TAI KHOAN THU NGHIEM' };
  await page.route('**/api/**', async route => {
    const url = new URL(route.request().url());
    let body: unknown;
    if (url.pathname === '/api/session') body = { ok: true };
    else if (url.pathname.startsWith('/api/products/')) body = { packId: 'friends-premium', priceVnd: 30000, currency: 'VND', available: true, priceVersion: 1 };
    else if (url.pathname === '/api/entitlements') body = { packIds: paid ? ['friends-premium'] : [], purchases: paid ? [{ packId: 'friends-premium', recoveryCode }] : [] };
    else if (url.pathname === '/api/orders') { expect(route.request().postDataJSON()).toEqual({ packId: 'friends-premium' }); expect(route.request().headers()['idempotency-key']).toBeTruthy(); body = { order }; }
    else if (url.pathname === '/api/orders/test-order') body = { order: { ...order, status: paid ? 'paid' : 'pending', ...(paid ? { recoveryCode } : {}) } };
