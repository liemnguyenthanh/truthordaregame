...
const generation: GeneratedPack = {
  id, status: 'complete', createdAt: '2026-09-17T12:00:00Z', mood: 'friendly',
  group: { id: 'offline-group', name: 'Nhóm offline thử nghiệm', players: [{ id: 'an', name: 'An' }, { id: 'binh', name: 'Bình' }] },
  pack: { id: `ai-${id}`, slug: `ai-${id}`, title: 'Bộ riêng offline', description: 'Seeded mock content for offline verification', tier: 'free', questionFile: '', contentVersion: '1', questionCount: 4, truthCount: 2, dareCount: 2, trialCount: 0, priceHintVnd: 0, productId: null, ageLabel: '16+', playerRange: { min: 2, max: 2 }, published: false, icon: '✨', color: 'purple', categoryIds: [] },
  questionSet: { schemaVersion: 1, packId: `ai-${id}`, locale: 'vi', contentVersion: '1', trialQuestionIds: [], questions: [
    { id: 't1', type: 'truth', text: 'An, kỷ niệm nào với Bình khiến bạn vui?', playerId: 'an', partnerId: 'binh' },
    { id: 't2', type: 'truth', text: 'Bình, bạn muốn cùng An đi đâu?', playerId: 'binh', partnerId: 'an' },
    { id: 'd1', type: 'dare', text: 'An, nói một lời khen dành cho Bình.', playerId: 'an', partnerId: 'binh' },
    { id: 'd2', type: 'dare', text: 'Bình, kể chuyện vui cho An nghe.', playerId: 'binh', partnerId: 'an' },
  ] },
};

test('SEEDED MOCK AI: real shell cache supports offline reload and a fresh page with correct actors', async ({ page, context }) => {
  const errors: string[] = [];
  ...
  await page.getByRole('button', { name: 'Lưu màn chơi để dùng bộ AI offline', exact: true }).click();
  await expect(page.getByRole('button', { name: 'Đã sẵn sàng chơi bộ này offline', exact: true })).toBeVisible({ timeout: 45000 });
  ...
  await expect(page.getByText('An, kỷ niệm nào với Bình khiến bạn vui?', { exact: true })).toBeVisible();
  await expect(page.getByText('Đã xem 1/4 câu', { exact: true })).toBeVisible();
  const fresh = await context.newPage();
  fresh.on('pageerror', error => errors.push(error.message));
  await fresh.goto(`/vi/bo-ai?id=${id}`);
  await expect(fresh.getByText('An, kỷ niệm nào với Bình khiến bạn vui?', { exact: true })).toBeVisible();
...
test('unconfigured AI reports AI availability honestly and never renders generated success', async ({ page, request }) => {
  ...
  await page.addInitScript(group => localStorage.setItem('tod:group:v1', JSON.stringify(group)), generation.group);
