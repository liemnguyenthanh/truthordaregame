...
try {
 ...
 console.log('PASS: migration, service-role RPCs, idempotency, restore, wrong amount, rollback/retry, rate limits, RLS/privilege assertions.');
} catch(error) {
 console.error({message:error.message,code:error.code,detail:error.detail,where:error.where});process.exitCode=1;
